# Feature Agent – Kubernetes

Deploy the Feature Agent to the same EKS cluster as Kafka and the Triage agent.

## Prerequisites

- Triage agent deployed and producing to `ticket.triaged.feature_request` (tickets classified as "feature_request")
...
ECR_URI="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/feature-agent:latest"
...
docker tag feature-agent:latest $ECR_URI
docker push $ECR_URI
```
...
kubectl create secret generic feature-agent-keys -n support-agents \
...
Edit `agents/feature/k8s/deployment.yaml` and set the image to your ECR URI. Then:

```bash
kubectl apply -f agents/feature/k8s/deployment.yaml
kubectl get pods -n support-agents -l app=feature-agent
kubectl logs -n support-agents -l app=feature-agent -f
```
